...
def load_snomed():
    df = pd.read_excel("../../NomenclaturePackAPI/ORPHA-SNOMEDCT_Mapping_File_production_2025.xlsx", skiprows=[1, 2])
...
index = "ElasticIndices/index_packorpha_en_product.txt"
try:
    del_req = es_server.indices.delete(index=index_output, error_trace=False)
...
for index_output in FileIndex:
    print(index_output)
    lang = index_output.split("_")[-1]
    index = f"./ElasticIndices/{index_output}.txt"
    try:
        del_req = es_server.indices.delete(index=index_output, error_trace=False)
        req = es_server.indices.create(index=index_output, body=body)
    except Exception as e:
